...
67.5 0 157.5 67.5;maptype:google;backgroundColor:#F5F3F0;zoom:4;tileStretchPixel:2;sliceZoom:5;maxZoom:20;minZoom:4;fullZoom:20;mapTilesWidth:2048;mapTilesHeight:1536;errorImg:/static/js/webgis/mxImg/blank.png;mapTilesExtendPixel:300"></div>

</template>
<script>
	import $ from 'jquery'
	import ytMapApi from '../../static/webgis/module/api.min.js'
	import '../../static/webgis/css/map.css'
	export default {
	    name: 'yt-map',
		data(){
			/*生成随机Id给dom*/
			const domId = 'map_' + (new Date().getTime()) + Math.random().toString().replace(/\./g, 'x');
			return {
				/* domId */
				domId: domId,
				/* mxLib对象 */
		...
		methods: {
			init(){
				...
				this.$emit('ready', map)
				/*map.addOverlay(new mxLib.Marker(map.getCenter(), {
					icon: new mxLib.Symbol(MX_Symbol_SHAPE_POINT, {
						scale: 2,//图标缩放大小
            ...
            addAlert(){
                ...
                var opts = {
                    ...
                    offset   : new mxLib.Size(30, -30)    //设置文本偏移量
                }
                var label = new mxLib.Label("欢迎使用元图地图，这是一个简单的文本标注哦~", opts);  // 创建文本标注对象
                label.setStyle({
                    color : "red",
                    border: 0,
                    background: 'transparent',
			...
			bindPointToMap (x, y, img, lableName, styleOptions) {
				...
				//添加lable
				var lable = this.addLable(point, lableName, Object.assign(this.defaultOption.labelStyle, styleOptions || {}));
				marker.label = lable;
				return marker;
			},
            /**
            ...
            addLable(position, lableName, labelStyle, offset, minZoom, maxZoom) {
                var px = 0;
                var l_length = lableName.length / 2;
                var radix = 16;
