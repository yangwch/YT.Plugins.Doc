...
export default {
    ...
    methods: {
         ...
         * */
        setSelected(value){
            if(this.multiple && ((value && value.length) || (this.defaultData && this.defaultData.length > 0))){
                this.$refs.tree.setCheckedKeys(value || this.defaultData, false);
            }else if(!this.multiple && (value || this.value)){
                let node = this.getNodeDataBykey(this.treeData, value || this.value);
        ...
        handleNodeClick(data, node) {
            if(!this.multiple){
                this.selectedLabel = node[this.comboProps.label];
